...
governmentServer <- function(input, output, session,current_theme = NULL) {
  ...
  output$officials_knowledge_regidor_plot <- renderPlotly({
    ...
    create_officials_knowledge_pie(participation_data()$responses, "Q5", "Regidor/a", active_theme())
  })
  
  output$officials_knowledge_sindico_plot <- renderPlotly({
    req(participation_data())
    create_officials_knowledge_pie(participation_data()$responses, "Q7", "Síndico/a", active_theme())
  })
  output$officials_knowledge_dipupadol_plot <- renderPlotly({
    req(participation_data())
    create_officials_knowledge_pie(participation_data()$responses, "Q8", "Diputado/a Local y/o Estatal", active_theme())
  })
  
  output$officials_knowledge_diputadof_plot <- renderPlotly({
    req(participation_data())
    create_officials_knowledge_pie(participation_data()$responses, "Q10", "Diputado/a Federal", active_theme())
  })
  
  # Card 2: Inequality Perception Plot
  output$inequality_perception_plot <- renderPlotly({
    req(perception_data())
    create_inequality_perception_pie(perception_data()$responses, active_theme())
  })
  
  # Card 3: Government Expectations Plot
  output$government_expectations_plot <- renderPlotly({
    req(participation_data())
    create_government_expectations_plot(participation_data()$responses, active_theme())
  })
  
  # Card 4: Important Problems Plot
  output$important_problems_plot <- renderPlotly({
    req(perception_data())
    create_important_problems_plot(perception_data()$responses, active_theme())
  })

...
prepare_ordinal_data <- function(data, question_id, metadata) {
  # Validate inputs
  ...
  # For special case questions, convert data
  if (special_cases) {
...
# Enhanced function to get the most common value with proper label
...
# Add function to create a label lookup table for a dataset
...
ordinalUI <- function(id) {
 ...
 # Add identical controls to the ordinal module
conditionalPanel(
...
ordinalServer <- function(id, data, metadata, selected_question, geo_data, current_theme = NULL) {
  moduleServer(id, function(input, output, session) {
    ...
    prepared_data <- reactive({
      ...
      prepare_ordinal_data(data(), selected_question(), metadata())
    })
    ...
    output$summary_stats <- renderPrint({
      ...
      cat("Estadísticas para Datos Ordinales:\n")
    ...
    output$district_map <- renderLeaflet({
      ...
      create_ordinal_district_map(
        ...
        use_gradient = input$use_color_gradient,  # Pass new option
        color_scale = input$color_scale,          # Pass color scale
